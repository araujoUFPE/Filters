...
#' @name LInSARRFE
#'
#' @description The Extension of the Refined Lee InSAR Filter (LInSARRFE) was
#'              developed based on the multilook phase difference expression
#'              'dFuncGierullEq7', implemented in the FDDPhase' package and
#'              defined in 'Closed-Form Expressions for InSAR Sample Statistics
#'              and Its Application to Non-Gaussian Data' for integer values
#'              of \code{L}, positive, and greater than \code{1}.
#'
#' @param imageRaster Phase difference image (interferogram) with values ranging from
#'          \code{-pi to pi}.
#'
#' @param eth Mask threshold to determine the maximum magnitude
#'            (defined empirically)
#'
...
LInSARRFE<-function(imageRaster,param,eth, xi=0.9){
  ...
  for (i in margen:(nrow(imageRaster) - margenm1)) {
    for (j in margen:(ncol(imageRaster) - margenm1)) {
      ...
      f <- function(janela_selecionada) {janela_selecionada * dFuncGierullEq7(janela_selecionada, param)}
      psi_bar <- integrate(f, lower = -psi_epsilon, upper = psi_epsilon, subdivisions = 100)$value

      f2 <- function(janela_selecionada){(janela_selecionada - psi_bar)^2 * dFuncGierullEq7(janela_selecionada, param)}
...
#' @name TcNfilter
#'
#' @description The Truncated Circular Normal Filter (TcNfilter) was
#'              developed based on the empirical Truncated Circular Normal
#'              model,implemented in the 'FDDPhase (function dTruncNorm)' package.
#'
#' @param imageRaster Phase difference image (interferogram) with values ranging from
#'          \code{-pi to pi}.
#'
...
TcNfilter<-function(imageRaster,param,eth, xi=0.9){
  ...
  for (i in margen:(nrow(imageRaster) - margenm1)) {
    for (j in margen:(ncol(imageRaster) - margenm1)) {
      ...
      f <- function(janela_selecionada) {janela_selecionada * dTruncNorm(janela_selecionada, param)}
      psi_bar <- integrate(f, lower = -psi_epsilon, upper = psi_epsilon, subdivisions = 100)$value

      f2 <- function(janela_selecionada){(janela_selecionada - psi_bar)^2 * dTruncNorm(janela_selecionada, param)}
      sigma_v_squared <- integrate(f2, lower = -psi_epsilon, upper = psi_epsilon, subdivisions = 100)$value
...
#' @name TcCfilter
#'
#' @description The Truncated Circular Cauchy Filter (TcCfilter) was
#'              developed based on the empirical Truncated Circular Cauchy
#'              model,implemented in the 'FDDPhase (function dTruncCauchy)'
#'              package.
#'
#' @param imageRaster Phase difference image (interferogram) with values ranging from
#'          \code{-pi to pi}.
